...
| **Captured private-war lords stay captured (AI side)** | `PrisonerReleaseCampaignBehavior.ReleasePartyPrisoners` — private sweep, CampaignBehavior | **AUTO-END risk #1.** On any peace/owner-change/load it frees prisoners whose faction is `!IsAtWarWith` the captor — and A↔C are never *formally* at war, so private-war prisoners auto-free. Triggers on `MakePeace`, `ClanChangedKingdom`, `OnSettlementOwnerChanged`, `OnGameLoaded`. No model hook. |
