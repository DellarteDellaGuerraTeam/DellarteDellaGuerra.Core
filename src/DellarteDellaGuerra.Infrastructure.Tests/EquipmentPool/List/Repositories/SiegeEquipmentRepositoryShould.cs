...
public class SiegeEquipmentRepositoryShould : TestFolderComparator
{
    ...
    {
        IList<Domain.EquipmentPool.Model.EquipmentPool> equipmentPools = Directory.EnumerateFiles(inputFolderPath).Select((filePath, poolId) =>
        {
            var equipmentPoolNodes = EvaluateFileXPath(filePath, "Equipments/*")
                .Select(node => new Domain.EquipmentPool.Model.Equipment(node))
                .ToList();
            return new Domain.EquipmentPool.Model.EquipmentPool(equipmentPoolNodes, poolId);
        }).ToList();
        ...
        characterEquipmentRepository
            ...
            .Returns(new Dictionary<string, IList<Domain.EquipmentPool.Model.EquipmentPool>>() {{ "vlandian_recruit", equipmentPools }});
