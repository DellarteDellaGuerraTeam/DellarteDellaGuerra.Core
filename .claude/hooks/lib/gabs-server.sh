...
echo "Starting GABS HTTP server on port ${GABS_HTTP_PORT}..."
...
powershell.exe -NoProfile -Command "
  ...
  Start-Process -FilePath '${GABS_EXE}' \
    -ArgumentList 'server --http localhost:${GABS_HTTP_PORT}' \
    -WindowStyle Hidden
"
...
if [ "${ready}" -eq 0 ]; then
  echo "WARNING: GABS HTTP server did not become ready within 15 s — bannerlord-game-controller MCP may be unavailable." >&2
fi
